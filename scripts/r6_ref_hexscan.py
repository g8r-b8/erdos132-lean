...
def make_K(ncent, phimax):
    ...
    for _ in range(ncent):
        ...
        b = np.array([x, -R + math.sqrt(R * R - x * x)]) - rng.uniform(0, 0.1) * np.array([0, 1])
...
stats, bad, ts = {}, [], {}
for it in range(NK):
    ncent = 0 if it == 0 else int(rng.integers(1, 4)); phimax = [0.02, 0.004, 0.02, 0.1][it % 4]
    W = make_K(ncent, phimax)
    xs = [0.0] if ncent == 0 else np.linspace(-0.6, 0.6, 13)
    for yx in xs:
