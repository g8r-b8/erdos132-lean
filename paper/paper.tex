...
The constant $C_0$ is explicit; the proof gives $C_0=3N_0+2\cdot10^8$ with $N_0=1.51\cdot 10^9$ (\cref{sec:assembly}). We have made no attempt to optimise it. A weaker bound, $\frac{54}{37}n+C_0$ with $C_0=3N_0+10^8$, obtained by a different argument in Regions II and III and with the trivial bound $\mult(\delta)\le e(S)+6|R|$ in place of \cref{sec:Rpoints}, is formalised in Lean~4; that argument is written out in \cref{app:lean54}, and the formalisation is described in \cref{sec:sat}. The bound $\frac{15}{11}$ is not formalised.
...
\emph{The main theorem.} The $\frac{54}{37}$ bound for Problem~1.6 of~\cite{CDL25}, proved in \cref{app:lean54}, is formalised in full in \texttt{E132Main*.lean}, \texttt{E132ConvexCurve.lean} and \texttt{E132Ve87.lean} (about 7{,}000 lines):
\begin{center}
...
\emph{Lean.} The whole development (all \texttt{E132*.lean} files: the convex-position results, the layer bound, Vesztergombi's and Hopf--Pannwitz's bounds, and the $\frac{54}{37}$ bound \texttt{erdos132\_main}) builds with \texttt{lake exe cache get} followed by \texttt{lake build}; on a laptop this takes about 6 minutes after the Mathlib cache is downloaded, of which about 4 minutes is \texttt{bv\_decide} for $n=13$. The toolchain is Lean \texttt{v4.35.0-rc2} with Mathlib.
